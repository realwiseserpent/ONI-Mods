...
{
    internal class PalmeraTreePatches_Translation
    {
        ...
        public class Localization_Initialize_Patch
        {
            ...
            public static void Translate(Type root)
            {
                ...
                Debug.Log($"{typeof(PalmeraTree).Namespace}: using translation done by {STRINGS.TRANSLATION.AUTHOR.NAME}");
            }
            ...
            private static void LoadStrings()
            {
                ...
                Debug.Log($"{typeof(PalmeraTree).Namespace}: Loading translation file: {path}");
                if (File.Exists(path))
                    OverloadStrings(LoadStringsFile(path, false));
                else
                    Debug.Log($"{typeof(PalmeraTree).Namespace}: Translation file not found, using default strings.");
            }
        }
    }
}
